...
for (yr in 1990:2024) {
  file <- paste0(dir_tmf, "/JRC_TMF_AnnualChange_v1_", yr, 
                 "_ASI_ID76_N20_E120.tif")
  if (!file.exists(file))
    paste0("https://ies-ows.jrc.ec.europa.eu/iforce/tmf_v1/download.py?",
...
area <- list.files("data/umrbpl/", full.names = TRUE, pattern = "shp$") |>
  read_sf() |> st_transform(crs = "EPSG:4326")
tmf <- tmf_files |> rast() |> crop(area) |> mask(area) |> 
  lapply(function(r) {
    levels(r) = names(data)
    return(r)
    }) |> rast()
...
for (i in seq_len(dim(tmf)[3])){
  for (c in 1:6){
    cat_mask = (tmf[[i]] == c)
    n_pixels = global(cat_mask, fun = "sum", na.rm=T)$sum
